...
class Extend_model(nn.Module):
	...
	def forward(self,x,cnn=None,fc=None):
		if cnn == -1:
			...
			x = torch.mul(x,torch.reshape(x_1,(x.shape[0], x.shape[1], 1, 1)))
			x = self.relu(x)
			x = self.max_pool(x)
...
def test_MultiNet(feature_model, model, gwr_model, test_loader, test_loader_size, number_of_tasks,num_classes_in_task,KNN_k,gwr=False,FC=False):
	class_div = 5
	model.eval()
	feature_model.eval()
	overall_corrects = 0
	...
	if gwr and FC:
		...
		for image_names, labels in test_loader:
			...
			pred_task = int(list(pred_tasks.keys())[sorted_matric[-1]//10])
			preds = int(sorted_matric[-1]%10)
...
for task_id in range(0, number_of_tasks):
	...
	if not FC_trained:
		...
		optimizer = optim.SGD([{'params':MultiNet.multi_cnns[str(task_id+1)].parameters()},\
			...
			{'params':MultiNet.SEfc1[str(task_id+1)].parameters(),'lr': 1e-4},\
			{'params':MultiNet.SEfc2[str(task_id+1)].parameters(),'lr': 1e-4}],\
...
if TestFC and TestGWR:
	gwr_overall,gwr_categorical_acc,mh_overall,mh_cl_bycl,nodes_per_task = test_MultiNet(resnet_model,MultiNet, gwr_model, test_loader, test_loader_size, number_of_tasks,num_classes_in_task,KNN_k,gwr=True,FC=True)
elif TestFC:
	mh_overall,mh_cl_bycl = test_MultiNet(resnet_model,MultiNet, gwr_model, test_loader, test_loader_size, number_of_tasks,num_classes_in_task,KNN_k,gwr=False,FC=True)
