...
# 				X.append(x_1)
# 			X = torch.stack(X,dim=1)
# 			X = torch.mul(torch.sign(X),torch.sqrt(torch.abs(X)))
# 			# X = self.relu(X)
# 			# print('X: ',X.shape)
# 			return X 
# 		elif cnn and fc:
...
def test_MultiNet(feature_model, model, gwr_model, test_loader, test_loader_size, number_of_tasks,num_classes_in_task,KNN_k,gwr=False,FC=False):
	...
	if gwr and FC:
		...
		for image_names, labels in test_loader:
			...
			output = output.cpu()
			pred_task = gwr_model.choose_task(output, number_of_tasks,KNN_k)
			# print('----------------------------')
			...
			if (pred_task == gwr_label) and ((preds[0]) == fc_label.data[0]):
				overall_corrects += 1
				predicted_class_by_class[pred_task*num_classes_in_task//class_div + preds[0]] += 1
...
if FC_trained:
	...
	name = 'FC_model/model_ds1_' + str(model_no) + suffix + '.pt'
...
if gwr_trained:
	name = 'FC_model/gwr_model_ds10_' + str(model_no) + suffix + '.pkl' # 'gwr_model_ds20_4.pkl' #  
...
for task_id in range(0, number_of_tasks):
	...
	if not FC_trained:
		...
		#optimizers and schedulers
		optimizer = optim.SGD([{'params':MultiNet.multi_cnns[str(task_id+1)].parameters()},{'params':MultiNet.multi_fcs[str(task_id+1)].parameters()}], lr = learning_rate, momentum = 0.9)
		scheduler = lr_scheduler.StepLR(optimizer, step_size = step_size, gamma = gamma)
...
if not FC_trained:
	torch.save(MultiNet.state_dict(),'FC_model/model_ds1_'  + str(model_no) + suffix + '.pt')

if not gwr_trained:
	name = 'FC_model/gwr_model_ds10_' + str(model_no) + suffix +'.pkl'
