...
def train_MultiNet(feature_model, model, criterion, optimizer, scheduler, train_loader, task_id, fc_num_epochs, gwr_epochs,num_classes_in_task,num_tasks,replay_data=None):
	...
	for epoch in range(1, fc_num_epochs + 1):
		...
		for image_names, task_labels,fc_labels in train_loader:
			...
			if replay_data:
				print('replay')
				replay_samples = torch.cat(([replay_data[img_idx+j][0] for j in range(sample_size)])).to(device)
				print('replay_samples: ', replay_samples)
				# feature_inputs = torch.cat((feature_inputs,replay_samples))
				replay_labels = torch.cat(([replay_data[img_idx+j][1] for j in range(sample_size)])).to(device)
...
resnet_model = models.resnet18(pretrained=True)#models.resnet18(pretrained=True)
...
for task_id in range(0, 50):
	...
	optimizer = optim.SGD([{'params':MultiNet.multi_cnns[str(head_id+1)].parameters()},\
	...
	if task_id in [9,19,29,39,49]:
		...
		# end_time1 = time.time()
		clf = KNeighborsClassifier(n_neighbors=20,weights='distance', p=1.2)#,algorithm='brute')
...
print('gwr_overall: ', np.sum(np.array(gwr_overall_list))/5)
print('mh_overall_list: ', np.sum(np.array(mh_overall_list))/5)
