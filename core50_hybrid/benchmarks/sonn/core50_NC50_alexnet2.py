...
class Extend_model(nn.Module):
	...
	def forward(self,x,cnn=None,fc=None,replay_data=None):
		if cnn == -1:
			...
			x = self.relu(x)
			x = x.view(-1, 256)  #(-1, self.out_features)
			if replay_data != None:
				x = torch.cat((x,replay_data))
			x = self.multi_fcs[str(int(fc))](x)
			x = self.multi_fcs2[str(int(fc))](x)
		elif cnn:
			x_2 = self.maxpool2(x)  #self.lp_pool(x)  #self.maxpool2(x)
			x = torch.reshape(x_2,(-1,256))
		elif fc:
			x = self.multi_fcs[str(int(fc))](x)
		return x
	...
	if gwr and FC:
		...
		for image_names, task_label,fc_label in test_loader:
			...
			if (pred_task == int(gwr_label)) and ((preds) == int(fc_label.data[0])):
				overall_corrects += 1
				predicted_class_by_class[pred_task*num_classes_in_task + preds.cpu()] += 1
		...
		for image_names, task_label,fc_label in test_loader:
			...
			_, preds = torch.max(output, 1)
			true_class_by_class[task_label*num_classes_in_task + fc_label.cpu()//class_div] += 1
			# print('preds: ',preds,' labels: ',labels)
			if (preds[0] == fc_label[0]):
				overall_corrects += 1
...
test = []
for i in range(50):
	test += CORe50_dataset.get_test_by_task(mode, i)[::10]
...
resnet_model = models.alexnet(pretrained=True)#models.resnet18(pretrained=True)
...
if FC_trained:
	for task_id in range(0, 10):
		...
		MultiNet.multi_fcs.update([[str(task_id+1), nn.Linear(256,256)]])
		MultiNet.multi_fcs2.update([[str(task_id+1), nn.Linear(256,num_fc_classes)]])
...
if not FC_trained:
	for i in range(1,11):
		...
		MultiNet.multi_fcs.update([[str(i), nn.Linear(256,256)]])
		MultiNet.multi_fcs2.update([[str(i), nn.Linear(256,num_fc_classes)]])
...
for task_id in range(0, 50):
	...
	optimizer = optim.SGD([{'params':MultiNet.multi_cnns[str(head_id+1)].parameters()},\
			...
			{'params':MultiNet.multi_fcs2[str(head_id+1)].parameters()}],\
...
# end_time1 = time.time()
clf = KNeighborsClassifier(n_neighbors=20,weights='distance', p=2)#,algorithm='brute')
