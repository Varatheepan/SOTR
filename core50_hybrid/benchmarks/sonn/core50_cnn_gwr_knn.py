...
class Extend_model(nn.Module):
	...
	def forward(self,x,cnn=None,fc=None):
		if cnn == -1:
			...
			x_2 = self.maxpool2(x)  # self.lp_pool(x)
			# x_3 = self.avgpool2(x)
			# x_2 = torch.mul(x_2,x_3)
...
def train_MultiNet(feature_model, model, criterion, optimizer, scheduler, train_loader, task_id, fc_num_epochs, gwr_epochs,num_classes_in_task,num_tasks):
	feature_model.eval()
	model.train()
...
def test_MultiNet(feature_model, model, clf, nca, test_loader, test_loader_size, number_of_tasks,num_classes_in_task,KNN_k,gwr=False,FC=False):
	...
	if gwr and FC:
		...
		for image_names, labels in test_loader:
			...
			output = output.cpu()
			# pred_task = int(list(gwr_model.choose_task(output, number_of_tasks,KNN_k).keys())[0])
			pred_task = clf.predict_proba(torch.reshape(output,(1,-1)))  #nca.transform(torch.reshape(output,(1,-1)))
			pred_acc_sorted,pred_task_sorted = torch.sort(torch.from_numpy(pred_task[0]), descending=True)
			...
			for p in range(2):
				with torch.set_grad_enabled(False):
					output = model(feature_inputs,cnn=pred_task_sorted[p]+1,fc=pred_task_sorted[p]+1)
				output = torch.nn.functional.softmax(output,dim=1)**4
				# print('output: ', output)
				# combined_matric.append(output[0]*pred_task_smax[p])
				combined_matric.append(output[0]*pred_acc_sorted[p])
			...
			pred_task = int(pred_task_sorted [sorted_matric[-1]//5])  #int(pred_task_sorted[0])#
			preds = int(sorted_matric[-1]%5)
		...
		for image_names, labels in test_loader:
			...
			if (preds[0] == fc_label.data[0]):
				overall_corrects += 1
				predicted_class_by_class[task_label*num_classes_in_task//class_div + preds[0]] += 1
...
test = test[::10] #reducing the frame rate 
...
# print(nn.Sequential(*list(resnet_model.children())))
# for name,parameters in resnet_model.named_parameters():
print('KNN bayesian || n: 8|| weight: \'distance\' || p: 2 || algorithm: \'auto\' || dataset- ::10')  #polynominal(2)- gamma: \'auto\'
print('without pca analysis transform & only fc at the end  || n_components = 200')  #svm gamma=\'auto\',kernel=\'rbf\'  with Linear discriminant analysis 
...
if FC_trained:
	...
	name = 'FC_model/model_dataset_full_' + str(model_no) + '_' + suffix + '.pt'
...
if gwr_trained:
	name = 'FC_model/gwr_model_ds10_' + str(model_no) + '_' + suffix + '.pkl'
...
if not FC_trained:
	torch.save(MultiNet.state_dict(),'FC_model/model_dataset_full_'  + str(model_no) + '_' + suffix + '.pt')
...
# end_time1 = time.time()
clf = KNeighborsClassifier(n_neighbors=8,weights='distance', p=2)#,algorithm='brute')
