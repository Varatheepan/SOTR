...
def gwr_features(feature_model,model,task_id,train_loader):
	...
	for image_names, labels in train_loader:
		...
		gwr_label = labels // num_classes_in_task
...
def train_MultiNet(feature_model, model, criterion, optimizer, tuned_optimizer, scheduler, train_loader, task_id, fc_num_epochs, gwr_epochs,num_classes_in_task,num_tasks):
	feature_model.eval()
	model.train()
	for epoch in range(1, fc_num_epochs + 1):
		...
		for image_names, labels in train_loader:
			...
			with torch.set_grad_enabled(True):
				outputs = model(feature_inputs,cnn=task_id+1,fc=task_id+1)
				# print(outputs.shape)
...
def test_MultiNet(feature_model, model, clf, nca, test_loader, test_loader_size, number_of_tasks,num_classes_in_task,KNN_k,gwr=False,FC=False):
	...
	class_names = ['Multi-plug','Cell phone','Scissors','Bulb','Can','Glasses','Ball','Highlighter','Mug','Remote']
	class_div = 5
	model.eval()
	feature_model.eval()
	overall_corrects = 0
	...
	if gwr and FC:
		...
		for image_names, labels in test_loader:
			...
			true_class_by_class[labels//class_div] += 1
			# preds[0] = preds[0]//class_div
			if (pred_task == int(gwr_label)) and ((preds) == int(fc_label.data[0])):
				overall_corrects += 1
				predicted_class_by_class[pred_task*num_classes_in_task//class_div + preds] += 1
			...
			# true_label = 'True label: ' + class_names[int(labels[0]//class_div)] 
			# sotr_pred =  'SOTR prediction: ' + class_names[int(pred_task*num_classes_in_task//class_div + preds)]
			# naive = 'Naive prediction: something'
			# ax.text(0.05,0.7,true_label,bbox=dict(boxstyle="square",ec=(0.5, 1, 0.5),fc=(0.5, 1, 0.5)))
			# ax.text(0.05,0.5,sotr_pred,bbox=dict(boxstyle="square",ec=(1, 0.5, 0.5),fc=(1, 0.5, 0.5)))
			# ax.text(0.05,0.3,naive)
			# plt.axis('off')
			# # ax.set_title(title)
			# plt.show()
...
# resnet_model.load_state_dict(torch.load('FC_model/tuned_resnet6.pt'))
...
if FC_trained:
	for task_id in range(0, number_of_tasks):
		...
		MultiNet.multi_fcs.update([[str(task_id+1), nn.Linear(out_features,num_fc_classes)]])
	...
	name = 'incremental_test_model/model_ds20_' + str(model_no) + '_' + suffix + '.pt'
	MultiNet.load_state_dict(torch.load(name))
	MultiNet = MultiNet.to(device)
	# print(MultiNet)

criterion = nn.CrossEntropyLoss()
tuned_optimizer = optim.SGD(resnet_model.parameters(), lr = 1e-3,weight_decay = 1e-4, momentum = 0.9)

if gwr_trained:
	name = 'incremental_test_model/gwr_model_ds20_' + str(model_no) + '_' + suffix + '.pkl'
...
for task_id in range(0, number_of_tasks):
	...
	if not FC_trained:
		...
		# print(MultiNet.parameters())
		# print(MultiNet.multi_cnns[str(task_id+1)].parameters())
		#optimizers and schedulers
		optimizer = optim.SGD([{'params':MultiNet.multi_cnns[str(task_id+1)].parameters()},\
			{'params':MultiNet.multi_fcs[str(task_id+1)].parameters()}],\
		...
		MultiNet = MultiNet.to(device)
		MultiNet = train_MultiNet(resnet_model, MultiNet, criterion, optimizer, tuned_optimizer, scheduler, train_loader, task_id, fc_num_epochs, gwr_epochs,num_classes_in_task,number_of_tasks)

		# if task_id == 0:
			# torch.save(resnet_model.state_dict(), 'FC_model/tuned_resnet6.pt')
	...
	if not gwr_trained:
		...
		if(task_id == 0):
			gwr_model.train(gwr_feature_tensor, gwr_labels, task_id, n_epochs = gwr_epochs,new_task = True)	# GWR initiation
		else:
			gwr_model.train(gwr_feature_tensor, gwr_labels,  task_id, n_epochs = gwr_epochs, new_task = True)	# continuing
	...
	# scaler = StandardScaler().fit(inputvectors)
	# inputvectors = scaler.transform(inputvectors)
	...
	clf = KNeighborsClassifier(n_neighbors=12,weights='distance', p=1.2)
	...
	gwr_overall_list.append(gwr_overall)
	gwr_cat_list.append(gwr_categorical_acc)
	mh_overall_list.append(mh_overall)
	mh_cat_list.append(mh_cl_bycl)

print('gwr_overall_list: ', gwr_overall_list)
print('gwr_cat_list: ', gwr_cat_list)
print('mh_overall_list: ', mh_overall_list)
print('mh_cat_list: ', mh_cat_list)

if not FC_trained:
	torch.save(MultiNet.state_dict(),'incremental_test_model/model_ds20_'  + str(model_no) + '_' + suffix + '.pt')
