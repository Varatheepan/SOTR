	...
	for epoch in range(1, num_epochs + 1):
		...
		for image_names, labels in train_loader:
			...
			with torch.set_grad_enabled(True):
				...
				if task_id != 0:
					loss += scaling_factor * surrogate_loss(model, previous_task_data,tuned_parameter)
					loss.backward()
					tuned_optimizer.step()
...
def test_gwr_fc(model, feature_model, gwr_model, criterion, test_loader, test_loader_size, number_of_tasks,
	...
	with torch.set_grad_enabled(False):
		for image_names, labels in test_loader:
			...
			model.state_dict()['fc.weight'].copy_(weight_dict[pred_task].to(device))  #vara
			model.state_dict()['fc.bias'].copy_(bias_dict[pred_task].to(device))
			outputs = model(inputs)
			_, preds = torch.max(outputs, 1)
