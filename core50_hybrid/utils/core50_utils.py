...
def test_model(model, criterion, optimizer, test_loader, test_loader_size, number_of_classes, transform = None):
	...
	test_loss = 0.0
	corrects = 0
	number_of_correct_by_class = torch.zeros((1, number_of_classes), dtype = int)
	number_by_class = torch.zeros((1, number_of_classes), dtype = int)
	confusion_matrix = torch.zeros((number_of_classes, number_of_classes), dtype = int)
	...
	for name, parameter in model.named_parameters():
		if parameter.requires_grad:
			...
			if task_id == 1:
				...
				big_omega += small_omega[name].cuda() / torch.add((torch.zeros(current_task_parameter.shape).cuda())**2, epsilon).cuda()
			else:
				prev_task_parameter = previous_task_data['{}_prev_task'.format(name)]
				big_omega = previous_task_data['{}_big_omega'.format(name)]
		...
		for image_names, labels in train_loader:
			...
			with torch.set_grad_enabled(True):
				...
				if task_id == 0:
					...
					si_loss.backward(retain_graph=True)
					tuned_optimizer.step()
					loss.backward()
					optimizer.step()
