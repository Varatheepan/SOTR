...
class CORe50(Dataset):
	...
	def get_train_by_task(self, mode, task_id):
		if mode == 'incremental_ni':
			...
			rough_start_index = no_of_objects*min_no_of_images_in_obj*(task_id)
			for i in range(rough_start_index , len(self.train_image_path_and_labels)):	# here rough_start_index is used to reduce the loop count
				if self.train_image_path_and_labels[i][1] == instances[task_id]:
					image_name = self.train_image_path_and_labels[i][0]
					label = self.train_image_path_and_labels[i][self.args.label_id]
					sample.append([image_name, label - 1])
				elif self.train_image_path_and_labels[i][1] > instances[task_id]:
					break
			...
			no_of_ses = 8
			min_no_of_images_in_obj = 292
			rough_start_index = no_of_ses*min_no_of_images_in_obj*self.args.shifter*(task_id)
			for i in range(rough_start_index, len(label_sorted_list)): 
				if label_sorted_list[i][self.args.label_id] == task_id + 1:
					image_name = label_sorted_list[i][0]
					label = label_sorted_list[i][self.args.label_id]
					sample.append([image_name, label - 1])
				elif label_sorted_list[i][self.args.label_id] == task_id + 2:
					break
			...
			sample = []
			min_no_of_images_in_obj = 292
			rough_start_index = min_no_of_images_in_obj*self.args.shifter*(task_id)
			for i in range(rough_start_index , len(self.train_image_path_and_labels)):
				if self.train_image_path_and_labels[i][self.args.task_label_id] == task_id +1:
					image_name = self.train_image_path_and_labels[i][0]
					label = self.train_image_path_and_labels[i][self.args.label_id]
					sample.append([image_name, label - 1])
				elif self.train_image_path_and_labels[i][self.args.task_label_id] == task_id + 2:
					break

		#for possible practical case
		elif mode == 'incremental_nc2':
			sample = []
			label_sorted_list = self.train_image_path_and_labels[self.train_image_path_and_labels[:,self.args.label_id].argsort()]
			no_of_ses = 8
			min_no_of_images_in_obj = 292
			...
			for i in range(rough_start_index, len(label_sorted_list)): 
				if label_sorted_list[i][self.args.label_id] == task_id + 1:
					image_name = label_sorted_list[i][0]
					task_label = label_sorted_list[i][3]
					class_label = label_sorted_list[i][2]
					sample.append([image_name, task_label - 1, (class_label - 1)%5])
				elif label_sorted_list[i][self.args.label_id] == task_id + 2:
					break
		return sample
